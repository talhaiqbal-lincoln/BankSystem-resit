...
namespace BankSystem
{
    public class Employee : Person
    {
        ...
        public bool createUser(string username, string password, string name, int age, string address, double balance, int account_lvl, string phonenum, string mail)
        {
            ...
            try
            {
                SqlCommand cmd = new SqlCommand("select * from [dbo].[User] where username = @username", con);
                cmd.Parameters.AddWithValue("@username", username);
                ...
                cmd = new SqlCommand("INSERT INTO [dbo].[User] (username, password, name, age, address, balance, sec_lvl, account_lvl, phonenum, mail) VALUES (@username, @password, @name, @age, @address, @balance, @sec_lvl, @account_lvl,@phonenum,@mail)", con);
                cmd.Parameters.AddWithValue("@username", username);
                cmd.Parameters.AddWithValue("@password", password);
        ...
        public void updateUser(string username, string address, int account_lvl, string phonenum, string mail)
        {
            ...
            con.Open();
            SqlCommand cmd = new SqlCommand("Update [User] set address=@address,account_lvl=@account_lvl,phonenum=@phonenum,mail=@mail where username=@username", con);
        ...
        public void deleteUser(string username)
        {
            ...
            string connection = "Data Source=localhost\\SQLEXPRESS;Initial Catalog=BankSystem;Integrated Security=True;Connect Timeout=30;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False";
            SqlConnection con = new SqlConnection(connection);
            con.Open();
            SqlCommand cmd = new SqlCommand("Delete [User] where  username=@username", con);
        ...
        public Customer? showData(string username)
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("select * from [dbo].[User] where username = @username and sec_lvl = 0", con);
                cmd.Parameters.AddWithValue("@username", username);
                con.Open();
